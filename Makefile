...
RUN_ARGS = $(SSH_AGENT_ARGS) $(DIND_ARGS) -v "$(PWD)":/workdir -w /workdir
DOCKER_RUN = docker run -it $(RUN_ARGS) $<

build-pre-commit:
	$(DOCKER_BUILD)

pre-commit: RUN_ARGS = --user "$(shell id -u):$(shell id -g)" \
           -w "$(HOME)" -v "$(PWD)":"$(HOME)" \
           -v "/etc/group:/etc/group:ro" \
           -v "/etc/passwd:/etc/passwd:ro" \
           -v "/etc/shadow:/etc/shadow:ro"
pre-commit: build-pre-commit ## run pre-commit
	$(DOCKER_RUN)
